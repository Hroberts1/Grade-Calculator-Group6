...
def on_course_button_click(root, course):
    ...
    view_course_info_button = tk.Button(course_menu_window, text="View Course Info", command=lambda: view_course_info(course))
...
def view_course_info(course):
    ...
    back_button = tk.Button(course_info_window, text="Back", command=course_info_window.destroy)
    back_button.pack()
...
style = ttk.Style()
style.configure('Hover.TButton', relief=tk.RAISED, borderwidth=3, foreground='black', background='lightgray', borderradius=10)
style.map('Hover.TButton', background=[('active', '!disabled', 'gray')])

update_main_menu()

button = ttk.Button(root, text="Add Course", command=on_button_click, style='Hover.TButton')
